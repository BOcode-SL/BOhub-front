...
| `/dashboard/leads`            | Leads list + sheet                          | admin, employee |
...
| `pdfJsPreview.ts`| 1 `getDocument`/blob + render page DPR (hub Campos + `/sign`); worker emit `.js` (Hostinger `.mjs` = text/plain) |
...
- **Billing**: Summary shows expanded KPIs + monthly chart. Payments/Expenses support installments (method Select, paidOn, notes). Payments: multi-línea `lines` (qty + P.U. + % dto → base derived); IVA/IRPF a nivel factura. Emit → PDF R2 **o** Confirmar cobro (sin factura) (`invoiceMode=none`, sin nº/PDF; badge lista). **Enviar al cliente** (`previewInvoiceSend` / `sendInvoice`, doble confirmación) aparte del emit (bloqueado si sin factura). Lista ingresos: filtro **Factura** (`invoice_filter` = draft|issued|no_invoice|no_number), no status ledger. Sin UI `reference`. Clientes: campo **provincia** (emit/PDF). Configuración: emisor/IBAN/numeración + plantilla email factura (`getInvoiceEmailTemplate` / `updateInvoiceEmailTemplate`). Payrolls tab.
...
- **Leads**: Lista paginada + tablero (DnD etapa). Sheet: Etapa/Asignado se guardan con **Guardar**; notas y board drag → PATCH inmediato. Admin+employee.
