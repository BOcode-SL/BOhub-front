...
export function TopProjectsByHours({ projects, formatHours, isLoading }: Props) {
    if (isLoading) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle className="text-base sm:text-lg">Top Proyectos por Horas</CardTitle>
                    <CardDescription className="text-xs sm:text-sm">Proyectos con más horas registradas este mes</CardDescription>
                </CardHeader>
                <CardContent>
                ...
                {projects.length > 0 ? (
                    <div className="space-y-3 sm:space-y-4">
                        {projects.map((project, index) => (
                            <div key={project.projectId} className="flex items-center justify-between gap-2 sm:gap-4">
                                <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
                    ...
                    <div className="py-8 text-center text-sm text-muted-foreground">No hay horas registradas este mes</div>
                )}
            </CardContent>
        </Card>
    );
