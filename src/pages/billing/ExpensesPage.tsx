...
export function ExpensesPage() {
  ...
  async function handleSave(data: ExpenseInput) {
    if (sheetMode === 'edit' && editing) await updateExpense(editing.id, data)
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Facturas recibidas y gastos del periodo.
          </p>
        </div>
        <Button
          type="button"
          ...
          <TableBody>
            ...
            {!loading && total === 0 && (
              <TableRow className="hover:bg-transparent">
                ...
                >
                  No hay gastos. Añade el primero.
                </TableCell>
              </TableRow>
            )}
            {rows.map((row) => (
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Soft delete del registro.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              variant="outline"
