...
export function MaintenanceSheet({
  ...
  useEffect(() => {
    ...
    if (period?.projectId === form.projectId && period.client) {
      ...
      setClientLabel(
        [c.name, c.email, c.phone].filter(Boolean).join(' · ') || c.name,
      )
      return
    }
    let cancelled = false
  ...
  function setPeriodOrStart(
    patch: Partial<Pick<MaintenanceInput, 'period' | 'startsOn'>>,
  ) {
    setForm((f) => {
      const next = { ...f, ...patch }
      if (!endsTouched) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="flex w-full flex-col gap-0 overflow-y-auto sm:max-w-md">
        <SheetHeader>
          <SheetTitle>
            {mode === 'add' ? 'Añadir mantenimiento' : 'Editar mantenimiento'}
          </SheetTitle>
          <SheetDescription>
            Mensual o anual. Contacto = datos del cliente. Renovar = nuevo
            periodo.
          </SheetDescription>
        </SheetHeader>
